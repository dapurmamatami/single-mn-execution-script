...
clonerepo() { #TODO: add error detection
	message "Cloning from github repository..."
  	cd ~/
	sudo git clone https://github.com/genesis-x/genesis-x.git
}

compile() {
	sudo chmod -R 777 genesis-x 
	cd genesis-x #TODO: squash relative path
...
createconf() {
	...
	printf "%s\n" "rpcuser=$rpcuser" "rpcpassword=$rpcpass" "rpcallowip=127.0.0.1" "listen=1" "server=1" "daemon=1" "maxconnections=256" > $CONFILE
        ...
        sleep 20s
        MNPRIVKEY=$(genesisx-cli masternode genkey)
	send-cli stop
...
success() {
	...
	message "MN $mnip:5555 $MNPRIVKEY TXHASH INDEX"
	exit 0
}
